...
# today=$(date +"%Y-%m-%d")
# automated_LM_fluentd_setup.sh 2>&1 | tee -a /tmp/LM_fluentd_install${today}.l>
...
^X Exit      ^R Read File ^\ Replace   ^U Paste Text^T To Spell  ^_ Go To Line
  GNU nano 4.8             automated_LM_fluentd_setup.sh
...
^G Get Help  ^O Write Out ^W Where Is  ^K Cut Text  ^J Justify   ^C Cur Pos
^X Exit      ^R Read File ^\ Replace   ^U Paste Text^T To Spell  ^_ Go To Line
  GNU nano 4.8             automated_LM_fluentd_setup.sh              Modified
printf -v s "%s\n" '
# provides UDP syslog reception for lm logs fluentd
module(load="imudp")
input(type="imudp" port="5140")
